...
\begin{frame}{Estrutura de arquivos (ilustrativa)}
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw<.(1)-> (root) |- (foo);
            \draw<.(1)-> (root) |- (bar);
            \node<.(2)->[draw=green, minimum width=5cm, fit={(foo) (bar)},
            below=4.5cm of root.west, anchor=west] {};
            \node<.(3)->[draw=red, minimum width=5cm, fit={(git/index)},
            below=2cm of root.west, anchor=west] {};
            \node<.(4)->[draw=blue, minimum width=5cm, fit={(git/repository)},
...
\begin{frame}{Workflow básico (comandos)}
    \begin{center}
        \begin{tikzpicture}
            ...
            \node<.(1)->[fill=green, minimum width=5cm] (wd) at (0,0) {Working directory};
            \node<.(1)->[fill=red, minimum width=5cm] (id) at (0,-2) {Index};
            \node<.(1)->[fill=blue, minimum width=5cm] (rp) at (0,-4) {Repository};
